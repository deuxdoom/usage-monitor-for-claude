...
- 트레이 메뉴에 "팝업 질감" 하위 메뉴를 추가해 팝업과 바의 표면을 매트와 글래스(베타) 중에서 고를 수 있습니다. 기본값은 매트이며, 글래스를 쓸 수 없는 Windows에서는 메뉴 항목이 비활성으로 표시됩니다. 선택은 `popup_material` 설정으로 저장되며, 고정했거나 바 모드로 열어 둔 팝업에도 2초 안에 반영됩니다
- 글래스 질감(베타)은 팝업 뒤의 화면이 실시간으로 비치는 한 장의 유리판으로 그립니다. 팝업 아래에 네이티브 층(`glass_layer.cs`, 빌드할 때 Windows 기본 .NET Framework 컴파일러로 컴파일)을 두고, Windows 합성기가 창 뒤의 내용을 3px 흐림, 채도 1.5배, 픽셀 밝기 상한 0.35로 GPU에서 직접 그리게 합니다. 화면을 복사하지 않으므로 앱은 화면 픽셀을 읽거나 저장하지 않고, 뒤에서 창이 움직이거나 영상·게임 화면이 바뀌어도 프레임마다 따라갑니다. 밝기 상한 덕분에 어둡거나 색이 강한 배경은 그대로 선명하게 비치고, 흰 창 위에서는 균일하게 눌려 모든 글자가 5.2:1 이상을 유지합니다(어두운 IDE, 흰 문서, Windows 기본 배경화면, 사진 위에서 실측). 창을 가로지르는 반사광, 마우스를 따라 맺히는 광택, 광택 있는 막대 채움, 은은하게 빛나는 퍼센트 숫자가 더해집니다. Windows 11 22H2 이상에서 쓸 수 있으며, Windows 설정의 "투명 효과"가 꺼져 있거나 네이티브 층을 만들지 못하면 그 팝업은 매트로 그립니다

### Changed

- 매트 질감에서 3.2.0의 상단 시그니처 색 번짐(방사형 그라디언트 두 개), 카드의 대각선 광택, 카드 그림자를 없앴습니다. 실제로 비치는 배경 없이 색만 번져 팝업이 뿌옇게 보였기 때문입니다
- 바 모드에서 시계 칸과 오른쪽 버튼 칸이 사용량 카드와 같은 배경을 쓰고, Claude와 Codex 카드 사이의 세로 구분선을 없애 바 전체가 한 장으로 보입니다. 3.2.0까지는 시계와 버튼 칸만 `--surface-quiet`의 회색빛으로 칠해져 남색 계열의 카드와 따로 놀았습니다
- 두 질감 모두 카드·계정·추가 사용량·설치 섹션, 탭, "한도 더 보기" 버튼의 1px 테두리를 없애고 채움만으로 구역을 나눠, 팝업이 조각난 카드 모음이 아니라 한 장의 표면으로 보입니다. 사용량 카드의 상태 색 왼쪽 선은 경고와 조회 지연 표시에 쓰이므로 그대로 둡니다
- Windows 11에서 팝업과 바의 모서리를 둥글게 하고, 테두리 선 없이 시스템 그림자로 창을 구분합니다. 3.2.0까지는 네모난 창 안쪽에 페이지가 1px 테두리를 직접 그렸으며, 그림자가 없는 Windows 10에서는 지금도 그렇게 합니다
- 바 모드 시계의 시와 분 사이 `:`가 Windows 시계의 초에 맞춰 1초에 한 번 깜빡입니다. 매초 앞 0.5초는 보이고 뒤 0.5초는 숨습니다. 3.2.0은 1초 켜짐과 1초 꺼짐을 반복해 2초에 한 번 깜빡였고, 바를 연 시각을 기준으로 돌아 실제 초가 바뀌는 순간과도 어긋났습니다
- 이 깜빡임은 Windows의 "애니메이션 효과"를 끈 환경에서도 동작합니다. 3.2.0까지는 이 설정이 동작 줄이기(`prefers-reduced-motion`)로 전달되면 콜론이 멈춰 있었습니다. 새로고침 아이콘 회전과 화면 전환 효과는 계속 이 설정을 따릅니다

### Fixed

- 팝업을 열고 닫을 때마다 시스템 자원이 조금씩 쌓이던 누수를 고쳤습니다. pywebview가 창마다 실행 파일 아이콘을 복제하고 해제하지 않아, 팝업을 한 번 열 때마다 아이콘 핸들(USER 객체 1개와 GDI 비트맵)이 프로세스가 끝날 때까지 남았습니다. 20번 열고 닫으면 GDI 객체가 34개에서 91개로 늘었고, 이제는 몇 번을 열어도 그대로입니다

[Show all code changes](https://github.com/deuxdoom/usage-monitor-for-claude/compare/v3.2.0...v3.3.0)
