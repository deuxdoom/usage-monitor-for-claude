...
class TestCertificateErrors(unittest.TestCase):
    ...
    requests raises SSLError as a subclass of ConnectionError, so without an
    explicit branch a company proxy inspecting TLS looks exactly like the
    ...
    def test_plain_connection_error_keeps_its_own_message(self, _mock_headers, mock_get):
        ...
        self.assertEqual(fetch_usage()['error'], EN['connection_error'])

    def test_os_certificate_store_is_injected(self):
        """The OS trust store replaces the bundled CA list at import time.

        Without it the app cannot reach the API from behind a TLS-inspecting
        corporate proxy, whose root only exists in the Windows store.
        """
        import ssl

        self.assertTrue(issubclass(ssl.SSLContext, truststore.SSLContext))
