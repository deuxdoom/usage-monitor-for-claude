...
* **포터블 (무설치):** 26MB 용량의 단일 EXE 파일로, 다운로드 후 바로 실행할 수 있습니다. 삭제하려면 파일을 지우기만 하면 됩니다.
...
* **상세 팝업 기능:** 트레이 아이콘을 클릭하면 계정 정보와 활성화된 모든 할당량(세션, 주간 한도 등), 추가 사용량, 초기화 시점을 상세히 볼 수 있습니다. 세션 한도는 초기화가 자정을 넘기더라도 항상 `3시간 20분 후 재설정`처럼 남은 시간으로 표시되어, 지금 작업을 이어갈 수 있는지 바로 판단할 수 있습니다. 반면 며칠 뒤에 풀리는 주간 한도는 정확한 날짜와 요일(예: `8월 20일(목)에 재설정, 14:00`)로 표시됩니다. 하단에는 다음 갱신까지 남은 시간이 카운트다운으로 표시되며, 수동 새로고침 버튼과 핀 고정 기능도 제공합니다.
...
* **이벤트 명령 실행:** 할당량이 초기화되거나 임계값을 넘었을 때, 앱 시작 시, 트레이 아이콘 더블 클릭 시 원하는 명령이나 스크립트를 자동으로 실행할 수 있습니다. 창이 뜨지 않고 백그라운드에서 조용히 동작합니다.
* **시간 마커 표시:** 현재 기간 내에 경과한 시간을 바(bar)에 표시하여, 시간 대비 사용량이 많을 경우 진행률이 빨간색으로 경고해 줍니다.
* **자동 토큰 갱신:** 세션이 만료되면 백그라운드에서 자동으로 토큰을 갱신합니다.
* **필요할 때만 도는 자동 갱신:** 팝업이 떠 있는 동안에는 계속 갱신하고, 팝업을 닫은 채 5분이 지나면 갱신을 멈춰 불필요한 API 호출을 만들지 않습니다. 팝업을 다시 열면 즉시 재개되며, 초기화 이벤트 명령을 설정해 두었다면 멈춘 상태에서도 초기화 시점에 맞춰 깨어나 명령을 제때 실행합니다. 사용량이 늘어나는 동안에는 더 자주 조회하고 초기화 시점에 맞춰 갱신 주기를 정렬합니다. 남은 시간 표시와 시간 마커는 API 조회와 무관하게 1분마다 스스로 갱신되므로, 조회가 늦어지거나 멈춰 있어도 화면의 시계는 계속 흐릅니다.
* **다중 계정 지원:** `--config-dir` 설정을 통해 여러 Claude 계정을 동시에 모니터링할 수 있습니다.
* **다국어 지원:** 한국어, 영어, 일본어를 지원하며 윈도우 시스템 언어에 맞춰 자동 적용됩니다(그 외 언어에서는 영어로 표시). 사용량 바의 기간 표기도 각 언어에 맞게 번역됩니다(예: `세션 (5시간)`, `주간 (7일)`).
...
* **디스크 쓰기 없음:** 파일 시스템에 어떤 파일도 생성하지 않으며, 윈도우 레지스트리에 알림 및 자동 시작 정보만 최소한으로 등록합니다. 1.80.0에서는 이름이 바뀌기 전에 쓰던 알림 등록 키 하나를 첫 실행 때 삭제합니다.
