...
* **필요할 때만 도는 자동 갱신:** 팝업이 떠 있는 동안에는 계속 갱신하고, 팝업을 닫은 채 5분이 지나면 갱신을 멈춰 불필요한 API 호출을 만들지 않습니다. 팝업을 다시 열면 즉시 재개되며, 초기화 이벤트 명령을 설정해 두었다면 멈춘 상태에서도 초기화 시점에 맞춰 깨어나 명령을 제때 실행합니다. 사용량이 늘어나는 동안에는 더 자주 조회하고 초기화 시점에 맞춰 갱신 주기를 정렬합니다.
