...
* **상세 팝업 기능:** 트레이 아이콘을 클릭하면 계정 정보와 활성화된 모든 할당량(세션, 주간 한도 등), 추가 사용량, 초기화 시점을 상세히 볼 수 있습니다. 초기화가 하루 이상 남았다면 남은 시간 대신 정확한 날짜와 요일(예: `8월 20일(목)에 재설정, 14:00`)을 표시하므로, 주간 한도가 언제 풀리는지 한눈에 알 수 있습니다. 수동 새로고침 버튼과 핀 고정 기능도 제공합니다.
...
* **적응형 폴링:** 사용량이 늘어나는 동안에는 자주, 유휴 상태에서는 드물게 조회하며 초기화 시점에 맞춰 갱신 주기를 정렬합니다. 불필요한 API 호출 없이 초기화 직후의 수치를 바로 반영합니다.
