...
* **끊기지 않는 자동 갱신:** 자리를 비우거나 화면을 잠가도 폴링이 멈추지 않으므로, 돌아왔을 때 오래된 수치를 보거나 갱신이 되살아나기를 기다릴 일이 없습니다. 사용량이 늘어나는 동안에는 더 자주 조회하고 초기화 시점에 맞춰 갱신 주기를 정렬하여, 불필요한 API 호출 없이 초기화 직후의 수치를 바로 반영합니다.
* **다중 계정 지원:** `--config-dir` 설정을 통해 여러 Claude 계정을 동시에 모니터링할 수 있습니다.
* **다국어 지원:** 한국어를 포함한 13개 언어(한국어, 영어, 독일어, 스페인어, 프랑스어, 힌디어, 인도네시아어, 이탈리아어, 일본어, 포르투갈어, 우크라이나어, 중국어 간체·번체)를 지원하며 윈도우 시스템 언어에 맞춰 자동 적용됩니다. 사용량 바의 기간 표기도 각 언어에 맞게 번역됩니다(예: `세션 (5시간)`, `주간 (7일)`).
